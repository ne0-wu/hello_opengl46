...
glm::vec2 to_glm(const Eigen::Vector2d &v)
{
    return {v[0], v[1]};
}
  ...
  public:
    static std::vector<MyGL::Vertex> vertices(const Mesh &mesh)
    {
        ...
        for (const auto &v : mesh.vertices())
        {
            ...
            const Eigen::Vector3d &normal = mesh.has_vertex_normals() ? mesh.normal(v) : zero3d;
            const Eigen::Vector2d &tex_coord = mesh.has_vertex_texcoords2D() ? mesh.texcoord2D(v) : zero2d;
